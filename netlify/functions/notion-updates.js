...
import { getStore, getDeployStore, setEnvironmentContext } from '@netlify/blobs';
...
export const handler = async (event) => {
  ...
  try {
    ...
    if (siteID && token) {
      store = getStore({ name: 'portal-updates', consistency: 'strong', siteID, token });
    } else {
      store = getDeployStore({ name: 'portal-updates', consistency: 'strong' });
    }
  } catch(e) {
    return respond(503, { error: 'Storage not available.' });
  }
