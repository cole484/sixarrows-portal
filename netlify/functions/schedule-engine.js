...
// Construction Scheduling Engine — CPM calculation, instantiation, validation
//
...
// ── Supabase helpers (same pattern as admin-clients.js) ─────
...
// Gates match CATS keys from selections-app.html.
// The 'sav' suffix stores { catKey: true } when a category is marked saved.

async function resolveGates(projectId) {
  const clientKey = projectId; // selections use client_id directly
  ...
  // Each gate checks whether the selection category is complete
  return {
...
async function runCPM(projectId) {
  ...
  const [client, allTasks, deps] = await Promise.all([
    sbFetch(`clients?id=eq.${encodeURIComponent(projectId)}&select=*`)
      .then(r => r?.[0]),
    sbFetch(
      `project_schedule?project_id=eq.${encodeURIComponent(projectId)}&order=sequence_order.asc`
    ),
    sbFetch(
      `task_dependencies?project_id=eq.${encodeURIComponent(projectId)}`
    ),
  ]);
  ...
  const targetEnd = client.target_end_date; // may be null

  // 2. Handle skipped tasks — rewire dependencies
  const skippedIds = new Set(
    allTasks.filter(t => t.status === 'skipped').map(t => t.id)
  );
  const tasks = allTasks.filter(t => t.status !== 'skipped');
  const taskMap = new Map(tasks.map(t => [t.id, t]));

  // Build raw dependency edges, then rewire around skipped
  let edges = (deps || []).filter(
    d => !skippedIds.has(d.task_id) || !skippedIds.has(d.blocked_by_task_id)
  );

  // For each skipped task: connect its predecessors to its successors
  for (const sid of skippedIds) {
    const predecessorsOfSkipped = (deps || [])
      .filter(d => d.task_id === sid)
      .map(d => d.blocked_by_task_id)
      .filter(id => !skippedIds.has(id));
    const successorsOfSkipped = (deps || [])
      .filter(d => d.blocked_by_task_id === sid)
      .map(d => d.task_id)
      .filter(id => !skippedIds.has(id));

    for (const succ of successorsOfSkipped) {
      for (const pred of predecessorsOfSkipped) {
        edges.push({ task_id: succ, blocked_by_task_id: pred });
      }
    }
  }

  // Remove edges involving skipped tasks
  edges = edges.filter(
    d => !skippedIds.has(d.task_id) && !skippedIds.has(d.blocked_by_task_id)
  );

  // 3. Build adjacency
  const predecessors = new Map(); // taskId → Set of predecessor taskIds
  const successors = new Map();   // taskId → Set of successor taskIds
  ...
  for (const t of tasks) {
    inDegree.set(t.id, predecessors.get(t.id).size);
  }
  ...
  while (queue.length > 0) {
    // Process in sequence_order when multiple are available
    ...
    for (const sId of successors.get(t.id)) {
      ...
      if (inDegree.get(sId) === 0) {
        queue.push(taskMap.get(sId));
      }
    }
  }
  ...
  for (const task of sorted) {
    // Completed tasks: pin to actuals
    ...
    // In-progress tasks: pin start to actual_start
  ...
  for (const task of sorted) {
    ...
    const daysToMSB = task.must_schedule_by
      ? daysBetween(now, task.must_schedule_by)
      : Infinity;
    ...
    if (daysToStart <= 7 && !task.contractor_confirmed) {
      ...
      alerts.red.push({
        ...
        reason: `Starts in ${daysToStart} days, contractor not confirmed`,
      });
    }
    ...
    if (task.selections_gate && !gateStatus[task.selections_gate]) {
      ...
      alerts.red.push({
        taskId: task.id, taskName: task.task_name,
        reason: `Selection gate "${task.selections_gate}" not resolved`,
      });
    }

    // Yellow: must_schedule_by within 14 days and no contractor assigned
    if (task.alert_level === 'none' && daysToMSB <= 14 && !task.assigned_contractor) {
      task.alert_level = 'yellow';
      alerts.yellow.push({
        taskId: task.id, taskName: task.task_name,
        reason: `Must schedule by ${task.must_schedule_by}, no contractor assigned`,
      });
    }
  }
...
async function handleInstantiate(projectId) {
  // 1. Load client
  const client = await sbFetch(
    `clients?id=eq.${encodeURIComponent(projectId)}&select=*`
  ).then(r => r?.[0]);
  ...
  const existing = await sbFetch(
    `project_schedule?project_id=eq.${encodeURIComponent(projectId)}&select=id&limit=1`
  );
  ...
  // 4. Filter by active workstreams
  ...
  // 6. Bulk insert schedule rows
  const inserted = await sbFetch('project_schedule', {
    method: 'POST',
    body: JSON.stringify(scheduleRows),
  });

  if (!inserted || inserted.length === 0) {
    return respond(500, { error: 'Failed to insert schedule rows' });
  }

  // 7. Build dependency edges
  // Map template_task_id → project_schedule.id for lookup
  const templateToSchedule = new Map();
  for (const row of inserted) {
    if (row.template_task_id) {
      templateToSchedule.set(row.template_task_id, row.id);
    }
  }
  ...
  for (const tmpl of filtered) {
    ...
    for (const blockerTemplateId of tmpl.default_blocked_by) {
      ...
      if (!blockerScheduleId) continue; // blocker was filtered out by workstream

      depRows.push({
        project_id: projectId,
        task_id: scheduleId,
        blocked_by_task_id: blockerScheduleId,
      });
    }
  }
  ...
  if (depRows.length > 0) {
    await sbFetch('task_dependencies', {
      method: 'POST',
      body: JSON.stringify(depRows),
    });
  }

  // 8. Run CPM calculation
...
async function handleGetSchedule(projectId) {
  const [client, tasks, deps] = await Promise.all([
    sbFetch(
      `clients?id=eq.${encodeURIComponent(projectId)}&select=id,client_name,build_start_date,target_end_date,computed_end_date,active_workstreams`
    ).then(r => r?.[0]),
    sbFetch(
      `project_schedule?project_id=eq.${encodeURIComponent(projectId)}&order=sequence_order.asc`
    ),
...
async function handleValidate(projectId) {
  ...
  if (!tasks || tasks.length === 0) {
    return respond(404, { error: 'No schedule found for project' });
  }
  ...
  for (const rule of SEQUENCING_RULES) {
    ...
    if (!beforeTask || !afterTask) continue; // task filtered out or skipped
    ...
    // Violation: "before" task finishes after "after" task starts
    if (beforeTask.planned_finish > afterTask.planned_start) {
      violations.push({
        rule: rule.message,
        beforeTask: rule.before,
        beforeFinish: beforeTask.planned_finish,
        afterTask: rule.after,
        afterStart: afterTask.planned_start,
      });
    }
  }
  ...
  return respond(200, {
    projectId,
    ruleCount: SEQUENCING_RULES.length,
...
async function handleUpdateTask(projectId, body) {
  ...
  const allowed = ['status', 'actual_start', 'actual_finish', 'assigned_contractor',
    'contractor_confirmed', 'notes', 'duration_days'];
  const patch = {};
...
export const handler = async (event) => {
  ...
  try {
    ...
    return respond(400, {
      error: 'Invalid action. Use: instantiate (POST), calculate (GET), schedule (GET), validate (POST), update-task (PATCH)',
    });
  } catch (err) {
    if (err.status) {
